...
<div class="flex justify-center gap-4 p-4">
    <label class="inline-flex items-center">
        <input type="checkbox" class="subcategoria" data-sub="ropa">
        <span class="ml-2">Ropa</span>
    </label>
    <label class="inline-flex items-center">
        <input type="checkbox" class="subcategoria" data-sub="tenis">
        <span class="ml-2">Tenis</span>
    </label>
    <label class="inline-flex items-center">
        <input type="checkbox" class="subcategoria" data-sub="accesorio">
...
<div class="flex flex-wrap justify-center gap-6 p-6">
    ...
    <a href="{{ route('producto.show', $loop->index + 1) }}" class="card
      @foreach($producto['categorias'] as $categoria) {{ $categoria }} @endforeach
      @foreach($producto['subcategorias'] as $subcategoria) {{ $subcategoria }} @endforeach
      bg-white border border-gray-200 rounded-lg shadow-md p-4 text-center w-48 hover:scale-105 transition-transform duration-300 ease-in-out hover:shadow-lg block"
      ...
      <p class="text-lg font-bold mt-2">${{ number_format($producto['precio'], 2) }}</p>
      <button class="bg-gray-800 text-white px-4 py-2 rounded-lg mt-4 hover:bg-gray-700">Ver detalles</button>
  </a>
    @endforeach
</div>
@endsection
